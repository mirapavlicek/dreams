...
//! Vykreslení celé palubovky. Souřadnice jsou v pixelech návrhu 480x800 a
//! RideLayout je přepočítá na skutečný displej.
class RideView extends WatchUi.View {
    ...
    hidden function panel(dc, x, y, width, height, radius) as Void {
    ...
    hidden function label(dc, x, y, text, size, colorName) as Void {
    ...
    hidden function number(dc, x, y, text, size, colorName) as Void {
        dc.setColor(RideLayout.color(colorName), Graphics.COLOR_TRANSPARENT);
        dc.drawText(RideLayout.x(x), RideLayout.y(y), RideLayout.numberFont(dc, size), text,
            Graphics.TEXT_JUSTIFY_CENTER | Graphics.TEXT_JUSTIFY_VCENTER);
    }

    hidden function bar(dc, x, y, width, height, ratio, colorName) as Void {
        var filled = ratio;
        if (filled < 0.0) {
            filled = 0.0;
        }
    ...
    hidden function drawClock(dc) as Void {
    ...
    hidden function cadenceColor(cadence) {
        if (cadence < 50) {
            return "textDim";
        }
        if (cadence <= 100) {
    ...
    hidden function drawCadence(dc) as Void {
    ...
    hidden function drawSpeed(dc) as Void {
    ...
    hidden function drawSpeedStats(dc) as Void {
    ...
    hidden function drawMiddle(dc) as Void {
    ...
    hidden function drawMetric(dc, x, y, width, height, title, value, unit, colorName, note, ratio) as Void {
        panel(dc, x, y, width, height, 10);
    ...
    hidden function drawCompass(dc, x, y, width, height) as Void {
        panel(dc, x, y, width, height, 10);
    ...
    // --- minimapa -----------------------------------------------------------
    //
    // Connect IQ neumí vykreslit mapové dlaždice do vlastního rozvržení
    // (MapTrackView zabírá celou obrazovku), takže tady kreslíme projetou
    // stopu z GPS bodů - drobečkovou mapu.

    hidden function drawMap(dc, x, y, width, height) as Void {
    ...
    hidden function drawTrack(dc, track as Lang.Array, x, y, width, height) as Void {
    ...
    hidden function drawBottom(dc) as Void {
    ...
    hidden function drawWeatherIcon(dc, x, y) as Void {
...
//! Klepnutí nebo prostřední tlačítko překreslí obrazovku okamžitě.
